...
namespace Server.Game
{
    ...
    public class Map
    {
        ...
        public bool CanGo(Vector2Int cellPos, bool checkObjects = true)
        {
            ...
            return _collision[y, x] != 1 && (!checkObjects || _objects[y, x] == null);
        }
        ...
        public bool IsEmptyCell(Vector2Int cellPos)
        {
            ...
            int x = cellPos.x - MinX;
            int y = MaxY - cellPos.y;
            return _objects[y, x] == null && _collision[y,x] == 0;
        }
        ...
        public void PrintCollisionAt(Vector2Int cellPos)
        {
            if (cellPos.x < MinX || cellPos.x > MaxX)
                return;
            if (cellPos.y < MinY || cellPos.y > MaxY)
            ...
            int x = cellPos.x - MinX;
            int y = MaxY - cellPos.y; // Adjusting Y coordinate to match internal storage which might be flipped

            // Check if coordinates are within the array bounds to avoid exceptions
            if (y >= 0 && y < _collision.GetLength(0) && x >= 0 && x < _collision.GetLength(1))
            {
                Console.WriteLine($"Collision at ({cellPos.x}({x}), {cellPos.y}({y})): {_collision[y, x]}");
            }
            else
            {
                Console.WriteLine("Invalid position: Coordinates out of the collision array range.");
            }
        }
